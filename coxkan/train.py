...
for disease in diseases:
    ...
    for idx0,seed in enumerate(seeds):
        print(f"seed:{seed}")
        set_seed(seed)
        K = 5
        ...
        with open(f'{prediction_root}/{disease}_seed{idx0}_train_mask.pkl', 'wb') as f:
            pkl.dump(train_mask, f)
        result_df.to_csv(f"{prediction_root}/{disease}_seed{idx0}_prediction.csv", index=False)
    ...
    with open(f"result.txt",'a') as f:
        ...
        f.write("\n")
    with open(f'{prediction_root}/{disease}_test_indices.pkl', 'wb') as f:
