...
class MyOwnDataset:
    ...
    def process(self):
        ...
        df_node = pd.read_csv(node_file, header=0,dtype={'patient_id': str})
...
def train(gene,adj_list,clinical,num_layers,save_path,log,seed=0,lr=1e-5,wd=1e-3,seed_n=0,list_path=None,file_path=None):
    ...
    train_mask_copy = train_mask.clone().numpy()
    test_mask_copy = test_mask.clone().numpy()
    ...
    best_model.eval()
    df_clinical = pd.read_csv(clinical, header=0)
    df_clinical = df_clinical.sort_values(by='patient_id')
    df_clinical = df_clinical.reset_index(drop=True)
    ...
    with torch.no_grad():
        ...
        prediction_output = out.cpu().numpy()
        res_df = pd.DataFrame({"patient_id":df_clinical['patient_id'].values.tolist(),
            "prediction_risk":prediction_output.tolist(),
    ...
    with open(list_path,'rb') as f:
        lst = pkl.load(f)
    lst.append(val_index1)
    with open(list_path,'wb') as f:
        pkl.dump(lst,f)
    with open(f'{file_path}_train_mask.pkl','wb') as f:
        pkl.dump(train_mask_copy,f)
    with open(f'{file_path}_test_mask.pkl','wb') as f:
        pkl.dump(test_mask_copy,f)
    res_df.to_csv(f'{file_path}_prediction.csv',index=False)
...
if __name__=='__main__':
    ...
    parser.add_argument('--seed_n',type=str,required=False,help="random seed of system default: 0")
    parser.add_argument('--list_path',type=str,required=False,help="path to append list of cindex")
    parser.add_argument('--file_path',type=str,required=False,help="path to save prediction files")

    args = parser.parse_args()
    seed_n = eval(args.seed_n)
    ...
    if not wd:
        ...
        wd = eval(wd)
    train(gene_path,adj_list_path,clinical_path,num_layers,model_save,log,seed=seed,lr=lr,wd=wd,seed_n=seed_n,list_path=list_path,file_path=file_path)
